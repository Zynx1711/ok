...
// --- CẬP NHẬT HITBOX ---
// HIT_RADIUS_FACTOR: Tỷ lệ diện tích va chạm so với kích thước chim (0.5 là toàn bộ hình vuông)
// Tăng lên 0.55 hoặc 0.6 nếu bạn muốn người chơi cảm thấy "bắn sượt qua cũng trúng"
const HIT_RADIUS_FACTOR = 0.48; 
// OFFSET của đầu mũi tên so với gốc tọa độ của nó (để mũi tên cắm vào chim thay vì trúng bằng đuôi)
const ARROW_TIP_OFFSET = 62; 
// -----------------------
...
function createBirds() {
  ...
  state.questionOrder.forEach((questionIndex, orderIndex) => {
    const birdSize = QUESTION_BIRD_SIZE - Math.random() * 8;
    const bird = {
      id: `q-${orderIndex}`,
      type: "question",
      questionIndex,
...
function spawnSingleBuff(buff, index = Date.now()) {
  const stageRect = stage.getBoundingClientRect();
  const birdSize = BUFF_BIRD_SIZE - Math.random() * 7;
  const bird = {
...
// --- LOGIC HITBOX MỚI: CHÍNH XÁC VÀ NHẠY HƠN ---
function checkArrowCollisions(arrow) {
  // 1. Xác định tọa độ chính xác của đầu mũi tên (Arrow Tip)
  // ArrowTipOffset giúp dịch chuyển điểm va chạm từ gốc div ra đúng đầu mũi nhọn của ảnh
  const arrowTipX = arrow.x + Math.cos(arrow.angle) * ARROW_TIP_OFFSET;
  const arrowTipY = arrow.y + Math.sin(arrow.angle) * ARROW_TIP_OFFSET;

  for (const bird of state.birds) {
    if (bird.hit) continue;
    
    // 2. Tọa độ tâm của con chim
    const birdCenterX = bird.x + bird.size / 2;
    const birdCenterY = bird.y + bird.size / 2;
    
    // 3. Tính khoảng cách từ đầu mũi tên đến tâm chim
    const dist = Math.hypot(arrowTipX - birdCenterX, arrowTipY - birdCenterY);
    
    // 4. Bán kính va chạm (Hitbox Circle)
    // Càng gần bird.size / 2 thì càng khít với hình ảnh
    const hitRadius = (bird.size / 2) * HIT_RADIUS_FACTOR;

    if (dist < hitRadius) {
      arrow.active = false;
      arrow.el.remove();
      hitBird(bird);
      return true; // Dừng vòng lặp vì mũi tên đã trúng
    }
  }
  return false;
}
